...
class PlayerViewController: UIViewController, AVPlayerItemMetadataOutputPushDelegate {
	...
	private func setUpPlayer(for asset: AVAsset) {
		...
		do {
			try mutableCompositionVideoTrack.insertTimeRange(CMTimeRangeMake(start: .zero, duration: asset.duration), of: sourceVideoTrack, at: .zero)
			if let sourceAudioTrack = sourceAudioTrack {
				try mutableCompositionAudioTrack.insertTimeRange(CMTimeRangeMake(start: .zero, duration: asset.duration), of: sourceAudioTrack, at: .zero)
			}
		}
		catch {
		...
		for metadataTrack in asset.tracks(withMediaType: .metadata) {
			if track(metadataTrack, hasMetadataIdentifier:AVMetadataIdentifier.quickTimeMetadataDetectedFace.rawValue) ||
				...
				let mutableCompositionMetadataTrack = mutableComposition.addMutableTrack(withMediaType: .metadata, preferredTrackID: kCMPersistentTrackID_Invalid)!
				
				do {
					try mutableCompositionMetadataTrack.insertTimeRange(CMTimeRangeMake(start: .zero, duration: asset.duration), of: metadataTrack, at: .zero)
				}
		...
		for metadataGroup in groups {
			...
			DispatchQueue.main.async {
				...
				if metadataGroup.items.count == 0 {
					if self.track(track!.assetTrack!, hasMetadataIdentifier: AVMetadataIdentifier.quickTimeMetadataDetectedFace.rawValue) {
						self.removeAllSublayers(from: self.facesLayer)
					}
					else if self.track(track!.assetTrack!, hasMetadataIdentifier: AVMetadataIdentifier.quickTimeMetadataVideoOrientation.rawValue) {
					...
					if self.honorTimedMetadataTracksDuringPlayback {
						...
						for metdataItem in metadataGroup.items {
							...
							switch itemIdentifier {
								case AVMetadataIdentifier.quickTimeMetadataDetectedFace:
								...
								case AVMetadataIdentifier.quickTimeMetadataVideoOrientation:
									if itemDataType == String(kCMMetadataBaseDataType_SInt16) {
										if let videoOrientationValue = metdataItem.value as? NSNumber {
											let sourceVideoTrack = self.playerAsset!.tracks(withMediaType: .video)[0]
